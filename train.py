...
"""
# python train.py --output_path outputs/E0.5new_reid0.5_w30000_ir --resume --name E0.5new_reid0.5_w30000 --gpu_ids 0,1
...
parser.add_argument('--output_path', type=str, default='.', help="outputs path, USEFUL IF STARTING FROM SCRATCH")
parser.add_argument('--name', type=str, default='latest_ablation', help="USEFUL ONLY IF RESUMING THE TRAINING OR LOADING PRE TRAINED WEIGHTS, outputs path")
...
if not opts.resume:
    ...
    train_writer = tensorboardX.SummaryWriter(os.path.join(opts.output_path + "/logs/config"))#opts.name))
