...
model_number = '00122000' # 00068431
...
image_datasets1 = datasets.ImageFolder(opts.input_folder+'/report_rgb', data_transforms)
image_datasets2 = datasets.ImageFolder(opts.input_folder+'/report_ir', data_transforms)
...
with torch.no_grad():
    for data, data2 in zip(dataloader_content, dataloader_structure): 
        ...
        output1 = pad(recover(generate(data[0], data2[0])), pad= npad) #rgb-ir
        output2 = pad(recover(generate(data2[0], data[0])), pad= npad)
        
        im[count] = np.concatenate((im1, w,  im2, w2, output1, w, output2, w2), axis=0)
        count +=1
        print(count)
...
pic.save(model_path+'/Swap_'+model_number+'.jpg')
