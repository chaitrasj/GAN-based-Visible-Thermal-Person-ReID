...
from reIDmodel import weights_init_kaiming, weights_init_classifier
...
class ContentEncoder(nn.Module):
    def __init__(self, n_downsample, n_res, input_dim, dim, norm, activ, pad_type, dropout, tanh=False, res_type='basic'):
        ...
        self.model_1 = []
        self.model_1 += [Conv2dBlock(dim, dim//2, 3, 1, 1, norm=norm, activation=activ, pad_type=pad_type)]
        self.model_1 += [Conv2dBlock(dim//2, 3, 3, 1, 1, norm=norm, activation=activ, pad_type=pad_type)]
        self.model_1 = nn.Sequential(*self.model_1)
        
        model_ft = models.resnet18(pretrained=True)
        model_ft.avgpool = nn.AdaptiveAvgPool2d((1,1))

        self.model_res_str = model_ft
        class_num = 395 #751
        self.classifier1 = ClassBlock(512, class_num, 0.5)
        self.classifier2 = ClassBlock(512, class_num, 0.75)
        
        self.model = nn.Sequential(*self.model)
        self.output_dim = dim
    
    def forward(self, x):
        s = self.model(x) #[4] Dont know whether i need to detach s
        
        x = self.model_1(s) #[0]
        x = self.model_res_str.conv1(x) #[1]
        x = self.model_res_str.bn1(x)
        x = self.model_res_str.relu(x)
        x = self.model_res_str.maxpool(x)
        x = self.model_res_str.layer1(x)
        x = self.model_res_str.layer2(x)
        x = self.model_res_str.layer3(x)
        x = self.model_res_str.layer4(x)
        x = self.model_res_str.avgpool(x)
        x = x.view(x.size(0), x.size(1))
        x1 = self.classifier1(x) #[2]
        x2 = self.classifier2(x) #[3]
        x=[]
        x.append(x1)
        x.append(x2)
        return s, x
