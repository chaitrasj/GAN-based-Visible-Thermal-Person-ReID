...
import random
import yaml
from utils import OriTripletLoss
...
class DGNet_Trainer(nn.Module):
    def __init__(self, hyperparameters, gpu_ids=[0]):
        ...
        # load teachers
        print('teacher hyperparameter = ',hyperparameters['teacher'])
        if (hyperparameters['teacher'] != "" and hyperparameters['teacher'] != None):
        ...
        self.criterion_triplet= OriTripletLoss(hyperparameters['batch_size'], hyperparameters['margin'])
    ...
    def forward(self, x_a, x_b, xp_a, xp_b):
        s_a, p_a = self.gen_a.encode(self.single(x_a))
        s_b, p_b = self.gen_b.encode(self.single(x_b))
        f_a = self.id_a(scale2(x_a))
        f_b = self.id_b(scale2(x_b))
        ...
        fp_a = self.id_a(scale2(xp_a))
        fp_b = self.id_b(scale2(xp_b))
        
        _, pp_a = self.gen_a.encode(self.single(xp_a))
        _, pp_b = self.gen_b.encode(self.single(xp_b))
        ...
        if self.erasing_p > 0:
            ...
            _, p_a = self.gen_a.encode(self.single(x_a_re))
            _, p_b = self.gen_b.encode(self.single(x_b_re))
            # encode the same ID different photo
            _, pp_a = self.gen_a.encode(self.single(xp_a_re)) 
            _, pp_b = self.gen_b.encode(self.single(xp_b_re))
    ...
    def gen_update(self, x_ab, x_ba, s_a, s_b, f_a, f_b, p_a, p_b, pp_a, pp_b, x_a_recon, x_b_recon, x_a_recon_p, x_b_recon_p, x_a, x_b, xp_a, xp_b, l_a, l_b, hyperparameters, iteration, num_gpu):
        ...
        if hyperparameters['use_encoder_again']>=rand_num:
            ...
            s_a_recon, p_a_recon  = self.gen_b.enc_content(self.single(x_ab_copy))
            s_b_recon, p_b_recon = self.gen_a.enc_content(self.single(x_ba_copy))
        else:
            # copy the encoder
            ...
            s_a_recon, p_a_recon = self.enc_content_copy(self.single(x_ab))
            s_b_recon, p_b_recon = self.enc_content_copy(self.single(x_ba))
        ...
        # encode again (encoder is fixed, input is tuned)
        f_a_recon = self.id_a_copy(scale2(x_ba))
        f_b_recon = self.id_b_copy(scale2(x_ab))

        # teacher Loss
        #  Tune the ID model
        log_sm = nn.LogSoftmax(dim=1)
        if (hyperparameters['teacher_w'] >0 and hyperparameters['teacher'] != "" and hyperparameters['teacher'] != None):
            if hyperparameters['ID_style'] == 'normal':
                _, p_a_student = self.gen_a.encode(self.single(x_ab_copy))
                p_a_student = log_sm(p_a_student)
                p_a_teacher = predict_label(self.teacher_model, scale2(x_ab_copy), num_class = hyperparameters['ID_class'], alabel = l_b, slabel = l_a, teacher_style = hyperparameters['teacher_style'])
                self.loss_teacher = self.criterion_teacher(p_a_student, p_a_teacher) / p_a_student.size(0)

                _, p_b_student = self.gen_a.encode(self.single(x_ba_copy))
                p_b_student = log_sm(p_b_student)
                p_b_teacher = predict_label(self.teacher_model, scale2(x_ba_copy), num_class = hyperparameters['ID_class'], alabel = l_b, slabel = l_a, teacher_style = hyperparameters['teacher_style'])
                ...
                _, p_ab_student = self.gen_a.encode(self.single(x_ab_copy))# s_a, f_b
                p_a_student = log_sm(p_ab_student[0])
                with torch.no_grad():
                    p_a_teacher = predict_label(self.teacher_model, scale2(x_ab_copy), num_class = hyperparameters['ID_class'], alabel = l_a, slabel = l_b, teacher_style = hyperparameters['teacher_style'])
                self.loss_teacher = self.criterion_teacher(p_a_student, p_a_teacher) / p_a_student.size(0)

                _, p_ba_student = self.gen_b.encode(self.single(x_ba_copy)) # s_b, f_a
                p_b_student = log_sm(p_ba_student[0])
                with torch.no_grad():
                    p_b_teacher = predict_label(self.teacher_model, scale2(x_ba_copy), num_class = hyperparameters['ID_class'], alabel = l_b, slabel = l_a, teacher_style = hyperparameters['teacher_style'])
                self.loss_teacher += self.criterion_teacher(p_b_student, p_b_teacher) / p_b_student.size(0)
                ...
                loss_B = self.id_criterion(p_ba_student[1], l_a) + self.id_criterion(p_ab_student[1], l_b)
        ...
        ########### ADDING TRIPLET LOSS ###############
        S1 = torch.cat((p_a[0],pp_a[0],p_b[0],pp_b[0]), 0)
        S2 = torch.cat((p_a[1],pp_a[1],p_b[1],pp_b[1]), 0)
        labels = torch.cat((l_a,l_a,l_b,l_b), 0)
        loss_tri1, batch_acc1 = self.criterion_triplet(S1, labels)
        loss_tri2, batch_acc2 = self.criterion_triplet(S2, labels)
        self.loss_triplet = loss_tri1 + loss_tri2
        self.acc_triplet = batch_acc1 + batch_acc2
        ...
        self.loss_gen_total = hyperparameters['gan_w'] * self.loss_gen_adv_a + \
                              ...
                              hyperparameters['triplet'] * self.loss_triplet
        if self.fp16:
            ...
            self.gen_opt.step()
            self.id_opt.step()
        print("L_total: %.4f, L_gan: %.4f,  Lx: %.4f, Lxp: %.4f, Lrecycle:%.4f, Lf: %.4f, Ls: %.4f, Recon-id: %.4f, id: %.4f, pid:%.4f, teacher: %.4f, triplet: %4f, trip acc: %4f"%( self.loss_gen_total, \
                                                        ...
                                                        hyperparameters['id_w'] * self.loss_id,\
                                                        hyperparameters['pid_w'] * self.loss_pid,\
hyperparameters['teacher_w'] * self.loss_teacher, hyperparameters['triplet'] * self.loss_triplet, self.acc_triplet )  )
    ...
    def sample(self, x_a, x_b):
        ...
        for i in range(x_a.size(0)):
            s_a, _ = self.gen_a.encode( self.single(x_a[i].unsqueeze(0)) )
            s_b, _ = self.gen_b.encode( self.single(x_b[i].unsqueeze(0)) )
            f_a = self.id_a( scale2(x_a[i].unsqueeze(0)))
            f_b = self.id_b( scale2(x_b[i].unsqueeze(0)))
            ...
            x_ab1.append(x_ab)
            #cycle
            s_b_recon, _ = self.gen_a.enc_content(self.single(x_ba))
            s_a_recon, _ = self.gen_b.enc_content(self.single(x_ab))
            f_a_recon = self.id_a(scale2(x_ba))
            f_b_recon = self.id_b(scale2(x_ab))
    ...
    def dis_update(self, x_ab, x_ba, x_a, x_b, hyperparameters, num_gpu):
        ...
        # D loss
        if num_gpu>1:
            self.loss_dis_a, reg_a = self.dis_a.module.calc_dis_loss(self.dis_a, x_ab.detach(), x_a)
            self.loss_dis_b, reg_b = self.dis_b.module.calc_dis_loss(self.dis_b, x_ba.detach(), x_b)
        else:
            self.loss_dis_a, reg_a = self.dis_a.calc_dis_loss(self.dis_a, x_ab.detach(), x_a)
            self.loss_dis_b, reg_b = self.dis_b.calc_dis_loss(self.dis_b, x_ba.detach(), x_b)
    ...
    def resume(self, checkpoint_dir, hyperparameters):
        ...
        print('Loading gen = ',last_model_name)
        ...
        print('Loading dis = ',last_model_name)
        state_dict = torch.load(last_model_name)
